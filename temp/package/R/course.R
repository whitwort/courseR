...
startAssignment <- function(name, overwrite = FALSE, path = getwd()) {
  courseR::startAssignment(name = name, overwrite = overwrite, path = path, pgk = system.file(package = "temp"))
}
...
checkAssignment <- function(name, path = getwd()) {
  courseR::checkAssignment(name = name, path = path, pkg = system.file(package = "temp"))
}
...
submitAssignment <- function(name, path = getwd()) {
  courseR::submitAssignment(name = name, path = path, pkg = system.file(package = "temp"))
}
...
checkAssignments <- function(path = getwd()) {
  courseR::checkAssignments(path = path, pkg = system.file(package = "temp"))
} 
